...
void init(int inicio, int final, int nodoActual) { 
    if( inicio == final ) {
        ...
        segmentTree[nodoActual].sum = 
            segmentTree[nodoIzquierdo].sum + segmentTree[nodoDerecho].sum;
        ...
        segmentTree[nodoActual].gcd = 
            gcd(segmentTree[nodoIzquierdo].gcd, segmentTree[nodoDerecho].gcd);
    } 
}
